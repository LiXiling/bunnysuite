...
Game-Frameworks sollen die Entwickler beim Implementieren der Game-Loop\footnote{\url{https://www.ssugames.org/old/pluginfile.php/1026/mod\_resource/content/1/2005_sbgames.pdf}} unterstützen. Diese Game-Loop besteht hauptsächlich aus 2 Phasen:
...
\begin{itemize}
\item Betrachten des leeren Himmels: Es werden keine Objekte auf dem Bildschirm dargestellt - \emph{Daggerfall, PC }
...
\begin{description}
\item[Validität] Dass sich die Kennzahl mit empirischen Messergebnissen deckt, ist garantiert, da sie empirisch zustande kommt. Es wurde die in Entwicklerkreisen übliche Größe {\em Renderzeit pro Frame in Sekunden} gewählt. Die Inverse dieser Größe, {\em Frames per Second}, ist zwar bei Nutzern und Marketing sehr beliebt, nicht jedoch bei technischen Entwicklern, unserer Zielgruppe, da die FPS-Zahl den Maßstab von Performance-Verlusten verschleiern. Fällt die FPS Zahl von 60 auf 50 und von 20 auf 10, so sind das jeweils 10 FPS Unterschied. In Renderzeit pro Frame lässt sich die tatsächliche Auswirkung leichter erkennen:Im ersten Fall erhöht sich die Renderzeit um 0,0033s, im zweiten Fall um 0,05s, also mehr als das 15-fache.\\
 Ein arbiträres Punktesystem würde die Messwerte ebenso nur unnötig verschleiern und den Eigenschaften eines guten Benchmarks widersprechen.\\
...
\subsection{Kha}
Kha ist ein open-source Framework, entwickelt von Robert Konrad. Es basiert auf Haxe und lässt sich auf eine Vielzahl von Plattformen und Engines portieren. Für den BunnySuite wurden die Tests in der eigenen IDE \textit{Kode} entwickelt und anschließend für die Zielplattform Windows in Visual Studio kompiliert. Mit Kha wurden unter anderem \emph{Spiral Ride} oder \emph{Dungeons Deep} realisiert.  Für die Darstellung der Bunnies wird die vom Framework bereitgestellte Klasse \emph{kha.Image} verwendet.
Für das Rendern von Text wird vom Framework aus einem bereitgestellten TrueType-Font ein Bitmap-Font generiert.
...
SDL ist im Vergleich zu den anderen Bibliotheken sehr low-level und stellt nur sehr grundlegende Funktionalitäten bereit. Um die Primitiven zu implementieren, wurde zusätzlich die Bibliothek \textit{SDL2\_gfx}\footnote{\url{http://www.ferzkopp.net/Software/SDL2\_gfx/Docs/html/index.html}} von Andreas Schiffler verwendet. Für die Darstellung der Bunnies wird die vom Framework bereitgestellte Klasse \emph{SDL\_Texture} verwendet. Für das Rendern von Text wird die Methode \emph{sdl2\_gfx.stringRGBA()} genutzt.

\subsection{XNA/Monogame}
XNA (XNA's Not Acronymed) ist ein von Microsoft entwickeltes Set von Tools zur Game Entwicklung. XNA basiert auf dem .NET Framework und läuft nativ auf Windows, Windows Phone und XBox360. Es war gedacht als einfache Alternative zu DirectX. XNA wird von Microsoft selber nicht mehr weiterentwickelt. Es exisitiert aber eine OpenSource Portierung namens Monogame, welche weiterhin aktiv gepflegt wird. Monogame läuft auch auf den Plattformen iOS, Mac OS, Android, Linux, Playstation 4 und PSVita. Beispiele für Spiele, die auf Monogame basieren, sind \emph{Fez} oder \emph{Transistor}. Um die Primitiven zu implementieren, wurde zusätzlich die Bibliothek \textit{Lilypath}\footnote{\url{https://github.com/jaquadro/LilyPath}} von Justin Aquadro verwendet. Für die Darstellung der Bunnies wird die vom Framework bereitgestellte Klasse \emph{Texture2D} verwendet.

\subsection{LibGDX}
LibGDX ist ein auf Java basierendes quelloffenes Framework. Es wurde entwickelt, da der Hauptentwickler beim programmieren von Android-Spielen schnell auf dem Desktop geschriebene Code-Änderungen testen wollte. Einer der Vorteile von LibGDX ist die Abstraktion der Unterschiede von Desktop-Plattformen wie Linux/Windows und Mobile-Plattformen wie Android/iOS. Beispiele für Spiele, die LibGDX einsetzen sind \emph{Animal Memory for children} oder \emph{Kid on Crack}. Für die Darstellung der Bunnies wird die vom Framework bereitgestellte Klasse \emph{gdx.graphics.Texture} verwendet.
...
\begin{figure}[!tbp]
  \centering
  \subfloat[Unterschreiten von 56 FPS.]{\includegraphics[width=0.5\textwidth]{bar_chart.png}\label{dia:bar}}
  \hfill
  \subfloat[Renderzeit pro Anzahl Objekte.]{\includegraphics[width=0.5\textwidth]{line_graph.png}\label{dia:line}}
...
\section{Die Parameter Repetitions und Schrittweite}
...
\begin{figure}[!tbp]
  \centering
  \subfloat[Schrittweite und Repetitions jeweils 1]{\includegraphics[width=0.5\textwidth]{fewrep.png}\label{dia:fewrep}}
  \hfill
  \subfloat[Schrittweite und Repetitions jeweils 50]{\includegraphics[width=0.5\textwidth]{manyrep.png}\label{dia:manyrep}}
